...
def refactor_solutions(input_path):
    ...
    # Path(__file__) is midterm2/scripts/refactor_fr_solutions.py
    # BASE_DIR is midterm2/ (the project root)
    BASE_DIR = Path(__file__).resolve().parent.parent
    
    # If no path is provided, default to the standard source location
    input_file = Path(input_path) if input_path else BASE_DIR / "source" / "fr-solutions.md"
    
    # Redirect directories to the root zones
    ...
    # --- YOUR ORIGINAL REFACTORING LOGIC (UNCHANGED) ---
    ...
    print(f"✅ Refactored solutions written to {build_dir}")

if __name__ == "__main__":
    # If a filename is passed as an argument, use it; otherwise use default
    arg_path = sys.argv[1] if len(sys.argv) > 1 else None
    refactor_solutions(arg_path)